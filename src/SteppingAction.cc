...
#include "G4Gamma.hh"
#include "G4LogicalVolumeStore.hh"  // Need this for GetInstance()
...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
  ...
  if (!fDetector1LV) {
    // Correct way to get the logical volume store and find volumes
  ...
  if (volume == fDetector1LV) {
    ...
    if (volume == fHeliumCloudLV) {
      ...
      track->SetMomentumDirection(newMomentum.unit());
